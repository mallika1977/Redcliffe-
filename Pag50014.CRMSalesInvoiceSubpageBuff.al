page 50014 "CRM Sales Invoice Subpage Buff"
{
    PageType = API;
    APIGroup = 'demo';
    APIVersion = 'v1.0';
    ...
    EntityName = 'CRMSalesInvoiceLine';
    EntitySetName = 'crmSalesInvoiceLines';

    ODataKeyFields = SystemId;
    SourceTable = "CRM Sales Invoice Line Buffer";
    ...
    layout
    {
        area(content)
        {
            repeater(GroupName)
            {
                ...
                field("RemoveLine"; Rec."Remove Line")
                {
                    ApplicationArea = All;
                }
                field(UID; Rec.UID)
    ...
    var
        ...
        RecCrmSInvHeader: Record "CRM Sales Invoice Header Buff";
        RecCRMSInvLine: Record "CRM Sales Invoice Line";
        RecCRMSInvLineBuff: Record "CRM Sales Invoice Line Buffer";
        LineNo: Integer;
        RecItem: Record Item;
    ...
    begin
        ...
        RecCrmSInvHeader.Reset();
        RecCrmSInvHeader.SetRange("CRM Invoice No.", Rec."CRM Invoice No.");
        IF not RecCrmSInvHeader.FindFirst() then
            Error('CRM Invoice No. must be same as header');
        ...
        IF Not RecCust.Get(Rec."Customer No.") then
            Error('Customer code not found %1', Rec."Customer No.");
        IF NOT ("Item Type" IN [0, 1]) then
            Error('Item Type must be 0 & 1, 0 for Item, 1 for Fixed Asset');

        IF Not RecItem.Get(Rec."Item Code") then
            Error('Item code not found %1', Rec."Item Code");

        IF Not RecbankAccount.Get(Rec."Bank Account Code") then
            Error('Bank Account code not found %1', Rec."Bank Account Code");

        IF NOT (Rec.Gender in [Rec.Gender::Male, Rec.Gender::Female, Rec.Gender::Other]) then
            Error('Gender code must be Male,Female or Other ,Current value is %1', Rec.Gender);

        IF Not (Rec."Remove Line" in [False, True]) then
            Error('Remove line value must be True or False. Current value is %1', Rec."Remove Line");

        RecCRMSInvLineBuff.Reset();
        RecCRMSInvLineBuff.SetRange("CRM Invoice No.", Rec."CRM Invoice No.");
        if RecCRMSInvLineBuff.FindLast() then
            Rec."Line No" := RecCRMSInvLineBuff."Line No" + 10000
        Else
            Rec."Line No" := 10000;


        RecCRMSInvLine.Reset();
        RecCRMSInvLine.SetRange("CRM Invoice No.", Rec."CRM Invoice No.");
        RecCRMSInvLine.SetRange(UID, Rec.UID);
        IF not RecCRMSInvLine.FindFirst() then begin
            RecCRMSInvLine.TransferFields(Rec);
            RecCRMSInvLine.Insert(true);
        end;

        RecCRMSInvLine.Reset();
        RecCRMSInvLine.SetRange("CRM Invoice No.", Rec."CRM Invoice No.");
        RecCRMSInvLine.SetRange(UID, Rec.UID);
        IF RecCRMSInvLine.FindFirst() then begin
            RecCRMSInvLine."Line No" := Rec."Line No";
            RecCRMSInvLine.Modify();
        end;
