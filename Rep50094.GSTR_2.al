report 50094 "GSTR-2"
{
    ...
    dataset
    {
        dataitem(Integer; Integer)
        {
            ...
            begin
                ...
                */
                MakeExcelBodyB2B();
                CreateExcelSheet_DD(B2BTxt, True);

                MakeExcelBodyIMPS();
                CreateExcelSheet_DD(IMPSTxt, False);

                MakeExcelBodyIMPG();
                CreateExcelSheet_DD(IMPGTxt, False);

                MakeExcelBodyB2BUR();
                CreateExcelSheet_DD(B2BURTxt, False);

                MakeExcelBodyCDNR();
                CreateExcelSheet_DD(CDNRTxt, False);

                MakeExcelBodyCDNUR();
                CreateExcelSheet_DD(CDNURTxt, False);

                MakeExcelBodyAT();
                CreateExcelSheet_DD(ATTxt, False);

                MakeExcelHeaderEXEMP();
                CreateExcelBufferExemp();
                CreateExcelSheet_DD(EXEMPTxt, False);

                MakeExcelBodyATADJ();
                CreateExcelSheet_DD(ATADJTxt, False);

                MakeExcelBodyHSNSUM();
                CreateExcelSheet_DD(HSNSUMTxt, False);

                MakeExcelBodyDoc();
                CreateExcelSheet_DD('Docs', false);
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Trade/Legal name', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        TempExcelBuffer.AddColumn('GSTR-1/IFF/GSTR-5 Period', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('GSTR-1/IFF/GSTR-5 Filing Date', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(EligibilityITCTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Reason', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Applicable % of Tax Rate', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Source', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('IRN', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('IRN Date', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
    end;
    ...
    var
        ...
        DateR: Record Date;
        TDSEn: Record "TDS Entry";
        TDSV: Decimal;
    begin
        DetGST.Reset();
        DetGST.SetRange("Document No.", GSTR2B2BQuery.Document_No_);
        if DetGST.FindFirst() then;
        TDSV := 0;
        TDSEn.Reset();
        TDSEn.SetRange("Document No.", GSTR2B2BQuery.Document_No_);
        IF TDSEn.FindFirst() THEN
            repeat
                TDSV += TDSEn."TDS Amount";
            UNTIL TDSEN.Next() = 0;
        TempExcelBuffer.NewRow();
        // TempExcelBuffer.AddColumn(GSTR2B2BQuery.Location__Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(DetGST."Buyer/Seller Reg. No.", false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        Vend.Reset();
        if Vend.get(DetGST."Source No.") then;
        ...
        PIH.reset;
        PIH.SetRange("No.", GSTR2B2BQuery.Document_No_);
        if PIH.FindFirst() then;
        TempExcelBuffer.AddColumn(PIH."Document Date", false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Date);

        PIH.reset;
        PIH.SetRange("No.", GSTR2B2BQuery.Document_No_);
        if PIH.FindFirst() then
            TempExcelBuffer.AddColumn(PIH."Vendor Invoice No.", false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
        ...
        TempExcelBuffer.AddColumn(GetInvoiceValue(GSTR2B2BQuery.Document_No_, "GST Document Type"::Invoice) + TDSV, false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        ...
        DateR.Reset();
        DateR.SetRange("Period Type", DateR."Period Type"::Month);
        DateR.SetRange("Period Start", CALCDate('-CM', GSTR2B2BQuery.Posting_Date));
        if DateR.FindFirst() then;
        TempExcelBuffer.AddColumn(DateR."Period Name" + ' ' + Format(Date2DMY(GSTR2B2BQuery.Posting_Date, 3)), false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(GSTR2B2BQuery.Posting_Date, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Date);
    ...
    local procedure MakeExcelBodyDoc()
    Var
        First_D: Code[30];
        Last_D: Code[30];
        Int: Integer;
        ...
        SalesCr: Record "Purch. Cr. Memo Hdr.";
        SalesInvH: Record "Purch. Inv. Header";
        SalesCrH: Record "Purch. Cr. Memo Hdr.";
    begin
        TempExcelBuffer.NewRow();
        TempExcelBuffer.AddColumn('Name of Document', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Sr. No. From', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Sr. No. To', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Total Number', false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        if SalesInv.findfirst then
            repeat
                if X_No <> SalesInv."No. Series" then begin
                    ...
                    SalesInvH.CopyFilters(SalesInv);
                    SalesInvH.SetRange("No. Series", SalesInv."No. Series");
                    Int := SalesInvH.Count;
                    if SalesInvH.FindFirst() then
                        First_D := SalesInvH."No.";
                    if SalesInvH.FindLast() then
                        Last_D := SalesInvH."No.";

                    TempExcelBuffer.NewRow();
                    TempExcelBuffer.AddColumn('Invoice of Outword Supply (' + Format(SalesInvH."Invoice Type") + ')', false, '', False, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    TempExcelBuffer.AddColumn(First_D, false, '', False, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    TempExcelBuffer.AddColumn(Last_D, false, '', False, false, false, '', TempExcelBuffer."Cell Type"::Text);
                    TempExcelBuffer.AddColumn(Int, false, '', False, false, false, '', TempExcelBuffer."Cell Type"::Number);
                    TempExcelBuffer.AddColumn(T_Can, false, '', False, false, false, '', TempExcelBuffer."Cell Type"::Number);

                end;
                X_No := SalesInv."No. Series";
            until SalesInv.next() = 0;

        SalesCr.Reset;
        SalesCr.SetCurrentKey("No. Series", "No.");
        SalesCr.Setrange("Posting date", Startdate, EndDate);
        if SalesCr.findfirst then
            repeat
                if X_No <> SalesCr."No. Series" then begin
                    First_D := '';
                    Last_D := '';
                    int := 0;
                    T_Can := 0;
                    ...
                    SalesCrH.Reset();
                    SalesCrH.SetCurrentKey("No.");
                    SalesCrH.CopyFilters(SalesCr);
                    SalesCrH.SetRange("No. Series", SalesCr."No. Series");
                    Int := SalesCrH.Count;
                    if SalesCrH.FindFirst() then
                        First_D := SalesCrH."No.";
                    if SalesCrH.FindLast() then
                        Last_D := SalesCrH."No.";
                ...
                X_No := SalesCr."No. Series";
            until SalesCr.next() = 0;
    ...
    var
        ...
        Vend: Record Vendor;
        DetGST: Record "Detailed GST Ledger Entry";
        PPCM: Record "Purch. Cr. Memo Hdr.";
    begin
        DetGST.Reset();
        DetGST.SetRange("Document No.", GSTR2CDNRQuery.Document_No_);
        if DetGST.FindFirst() then;

        PPCM.Reset();
        PPCM.SetRange("No.", GSTR2CDNRQuery.Document_No_);
        if PPCM.FindFirst() then;
        TempExcelBuffer.NewRow();
        ...
        if Vend.get(DetGST."Source No.") then;
        TempExcelBuffer.AddColumn(Vend.Name, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(PPCM."Document Date", false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Date);
