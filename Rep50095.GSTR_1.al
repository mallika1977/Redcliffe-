report 50095 "GSTR-1 Report"
{
        ...
        dataitem(Integer; Integer)
        {
            ...
            begin
                ...
                */
                ...
                B2CLimit := 250000;
                MakeExcelBodyB2CL();
                CreateExcelSheet_DD(B2CLTxt, false);

                MakeExcelBodyB2CS();
                CreateExcelSheet_DD(B2CSTxt, false);
                ...
                MakeExcelBodyEXP();
                CreateExcelSheet_DD(EXPTxt, False);

                MakeExcelBodyHSN();
                CreateExcelSheet_DD(HSNTxt, False);

                MakeExcelBodyEXEMP();
                CreateExcelSheet_DD(EXEMPTxt, false);

                MakeExcelBodyDoc();
                CreateExcelSheet_DD('Doc', false);
    ...
    begin
        ...
        if GSTR1B2BQuery.Source_No_ <> '' then begin
            if Cust.get(GSTR1B2BQuery.Source_No_) then;
            TempExcelBuffer.AddColumn(Cust.Name, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        end else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        /*
        if GSTR1B2BQuery.Source_No_ <> '' then//ANI ADDED(field notfound in base GSTR1B2BQuery.Name )
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text)
        else
            TempExcelBuffer.AddColumn('', false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
*/
        ...
        Rec_DGST.SetRange("Document No.", GSTR1B2BQuery.Document_No_);
        Rec_DGST.SetRange("GST Customer Type", GSTR1B2BQuery.GST_Customer_Type);
        if Rec_DGST.FindSet() then begin
            repeat
                if Rec_DGST."GST Component Code" = 'IGST' then
                    IGST_AMT += ABS(Rec_DGST."GST Amount");
                if Rec_DGST."GST Component Code" = 'CGST' then
                    CGST_AMT += ABS(Rec_DGST."GST Amount");
                if Rec_DGST."GST Component Code" = 'SGST' then
                    SGST_AMT += ABS(Rec_DGST."GST Amount");
            until Rec_DGST.Next() = 0;
        end;
        TempExcelBuffer.AddColumn(Abs(IGST_AMT), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        TempExcelBuffer.AddColumn(Abs(CGST_AMT), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
        TempExcelBuffer.AddColumn(Abs(SGST_AMT), false, '', false, false, false, '0.00', TempExcelBuffer."Cell Type"::Number);
